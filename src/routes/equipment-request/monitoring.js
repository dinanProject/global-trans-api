...
function buildActionHistoryDescription({ transition, remarks, reviewSchedule }) {
...
function validateAssignmentPayload(payload) {
  ...
  if (payload.plannedEndDate < payload.plannedStartDate) {
    return {
      valid: false,
      message: 'Planned end date tidak boleh lebih kecil dari planned start date.',
    };
  }
...
async function validateEquipmentSchedule(trx, payload) {
  ...
  if (overlap) {
    return {
      valid: false,
      message: 'Equipment unit sudah memiliki assignment aktif pada periode tersebut.',
    };
  }
...
function calculateRemainingDays(plannedEndDate, actualEndDate = null) {
  ...
  const comparisonDate = actualEndDate ? normalizeDateOnly(actualEndDate) : getTodayDate();
  ...
  return Math.ceil((plannedEnd.getTime() - currentDate.getTime()) / (24 * 60 * 60 * 1000));
}
...
function calculateAssignmentMonitoring(assignment) {
  ...
  return {
    operationStatus,
    slaStatus,
    remainingDays: calculateRemainingDays(plannedEndDate, assignment.actualEndDate),
...
function normalizeMonitoringFilters(query = {}) {
  ...
  if (query.overdueOnly !== null && query.overdueOnly !== undefined && query.overdueOnly !== '') {
    try {
...
function applyMonitoringFilters(query, filters, aliases = {}) {
  ...
  if (filters.startDate) {
    query.andWhere(`${assignmentAlias}.plannedEndDate`, '>=', filters.startDate);
  }

  if (filters.endDate) {
    query.andWhere(`${assignmentAlias}.plannedStartDate`, '<=', filters.endDate);
  }
}
...
router.get('/overview', authorization('EQUIPMENT_MONITORING.VIEW'), async (req, res) => {
  try {
    ...
    const availableUnitQuery = db('equipmentUnits as equipmentUnit')
      ...
      .where('equipmentUnit.operationalStatusCode', 'AVAILABLE')
    ...
    const maintenanceUnitQuery = db('equipmentUnits as equipmentUnit')
      .where('equipmentUnit.isActive', true)
      .where('equipmentUnit.operationalStatusCode', 'MAINTENANCE')
      .whereNull('equipmentUnit.deletedAt')
      .count({ total: 'equipmentUnit.id' })
      .first();

    const [assignmentRows, availableUnitResult, maintenanceUnitResult] = await Promise.all([
      query.orderBy([
        {
    ...
    return res.fail(error.message || 'Failed to load equipment monitoring overview.');
  }
});
/**
 ...
 * - endDate
 */
router.get('/summary', authorization('EQUIPMENT_MONITORING.VIEW'), async (req, res) => {
  try {
    const access = await getRequestAccess(req);
    const filters = normalizeMonitoringFilters(req.query);
 ...
 * - overdueOnly
 */
router.get('/assignments', authorization('EQUIPMENT_MONITORING.VIEW'), async (req, res) => {
  try {
    const access = await getRequestAccess(req);
    const filters = normalizeMonitoringFilters(req.query);
    ...
    console.error('GET /equipment-request/monitoring/assignments error:', error);

    return res.fail(error.message || 'Failed to load monitoring assignments.');
  }
});
...
router.get('/:uuid', authorization('EQUIPMENT_REQUEST.VIEW'), async (req, res) => {
  try {
    ...
    const assignment = await findMonitoringAssignmentByUuid(req.params.uuid, access);

    if (!assignment) {
      return res.incomplete('Equipment assignment monitoring tidak ditemukan.');
    }
    ...
    return res.fail(error.message || 'Failed to load equipment assignment monitoring.');
  }
});
...
async function findMonitoringAssignmentByUuid(uuid, access, trx = db) {
  const query = buildMonitoringAssignmentQuery(trx)
    .leftJoin('users as assignedUser', function () {
      this.on('assignedUser.id', '=', 'assignment.assignedBy').andOnNull('assignedUser.deletedAt');
    })
    .select([
      'assignment.id',
      'assignment.uuid',

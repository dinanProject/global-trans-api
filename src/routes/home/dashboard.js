...
const DASHBOARD_CHART_FEATURE_CODE = 'SHOW_DASHBOARD_CHART';
...
function isEnabledConfigValue(value) {
  const normalizedValue = normalizeNullableString(value)?.toUpperCase();

  return ['Y', 'YES', '1', 'TRUE'].includes(normalizedValue);
}

async function getDashboardConfig(trx = db) {
  const config = await trx('appConfigs').select(['code', 'value']).where('code', DASHBOARD_CHART_FEATURE_CODE).first();

  return {
    showDashboardCharts: isEnabledConfigValue(config?.value),
  };
}
...
function calculateSlaStatus(assignment) {
  ...
  if (actualEndDate) {
    return plannedEndDate && actualEndDate <= plannedEndDate ? 'COMPLETED_ON_TIME' : 'COMPLETED_LATE';
  }
  ...
  if (actualStartDate) {
    ...
    return plannedStartDate && actualStartDate > plannedStartDate ? 'LATE_START' : 'ON_TIME_START';
  }
...
function buildPeriodBuckets(periodCode, range) {
  ...
  const cursor = new Date(Date.UTC(range.startDate.getUTCFullYear(), range.startDate.getUTCMonth(), 1));

  const endMonth = new Date(Date.UTC(range.endDate.getUTCFullYear(), range.endDate.getUTCMonth(), 1));
...
async function getOperationsTrend(access, filters, periodCode, range, trx = db) {
  const query = buildAssignmentDashboardQuery(trx)
    .select(['assignment.id', 'assignment.assignedAt', 'assignment.actualStartDate', 'assignment.actualEndDate'])
    .andWhere((builder) => {
      builder
        .whereBetween('assignment.assignedAt', [range.startDate, range.endDate])
        .orWhereBetween('assignment.actualStartDate', [range.startDate, range.endDate])
        .orWhereBetween('assignment.actualEndDate', [range.startDate, range.endDate]);
    });
...
async function getEquipmentUtilization(access, filters, trx = db) {
  const currentAssignmentQuery = buildAssignmentDashboardQuery(trx)
    .select(['assignment.equipmentUnitId', 'assignment.actualStartDate', 'assignment.actualEndDate'])
...
async function getSlaPerformance(access, filters, range, trx = db) {
  const query = buildAssignmentDashboardQuery(trx)
    .select(['assignment.plannedStartDate', 'assignment.plannedEndDate', 'assignment.actualStartDate', 'assignment.actualEndDate'])
    .whereBetween('assignment.plannedStartDate', [range.startDate, range.endDate]);
...
router.get('/config', async (req, res) => {
  try {
    ...
    return res.fail(error.message || 'Failed to load dashboard config.');
  }
});
...
router.get('/', async (req, res) => {
  try {
    ...
    if (!period) {
      return res.incomplete('Period harus berupa 30_DAYS, 6_MONTHS, atau 12_MONTHS.');
    }
    ...
    const [operationsTrend, equipmentUtilization, slaPerformance, topEquipmentCategories, operationsByCompany] = await Promise.all([
    ...
    return res.fail(error.message || 'Failed to load equipment operations dashboard.');
  }
});

...
router.post(
  ...
  (req, res, next) => {
    if (!upload)
      return res.fail(
        "Dependency multer belum terpasang. Jalankan: npm install multer",
      );
  ...
  async (req, res) => {
    ...
    try {
      ...
      await trx.commit();
      return res.success(
        await db("fileAttachments").where("uuid", uuid).first(),
      );
    } catch (error) {
      await trx.rollback();
...
router.get("/", authorization("FILE_ATTACHMENT.VIEW"), async (req, res) => {
  try {
    const query = db("fileAttachments as file")
      ...
      .select([
        "file.*",
        "uploader.uuid as uploadedByUuid",
    ...
    if (req.query.documentType)
      query.where("file.documentType", req.query.documentType);
...
router.get(
  "/:uuid/download",
  authorization("FILE_ATTACHMENT.VIEW"),
  async (req, res) => {
    try {
      ...
      if (!row || !fs.existsSync(row.filePath))
        return res.incomplete("File tidak ditemukan.");
...
router.delete(
  "/:uuid",
  authorization("FILE_ATTACHMENT.DELETE"),
  async (req, res) => {
    try {

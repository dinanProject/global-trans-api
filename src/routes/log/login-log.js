...
route.get('/', authentication, authorize('LOGIN_LOG.VIEW'), async function (req, res, next) {
  try {
    ...
    const SYSTEM_DEVELOPER_UUID = 'aa9fc02c-8799-11f1-9650-1e30aa2c7567';

    const query = db('userLoginLogs as logs')
      .leftJoin('sysLookups as statuses', 'statuses.lookupId', 'logs.loginStatusId')
      .leftJoin('users as users', 'users.id', 'logs.userId')
      .where('statuses.lookupGroup', 'LOGIN_STATUS')
      .andWhere(function () {
        this.whereNull('users.uuid').orWhereNot('users.uuid', SYSTEM_DEVELOPER_UUID);
      });
    ...
    if (dateTo) {
      query.andWhere('logs.createdAt', '<', db.raw('DATE_ADD(?, INTERVAL 1 DAY)', [dateTo]));
    }
